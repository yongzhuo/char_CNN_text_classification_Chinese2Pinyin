...
class Dataset(object):
    ...
    def dataset_read(self):
        ...
        classesfile = open('data/ag_news_csv/classes.txt', 'r', encoding='utf-8')
        while True:
            try:
                ...
                if not line:
                    break
                labels = line.strip()
                labeltext = labels.split(' ')
                labeldict[labeltext[0].strip()] = labeltext[1].strip()
            except:
                print('read ', 'classesfile', ' wrong')

        txtfile = open(self.data_source, 'r', encoding='utf-8')
        while True:
            try:
                ...
                content = sentencetext[1]
                docs.append(content)
                label.append(labeldict[sentencetext[0]])
                doc_count = doc_count + 1
            except:
                print('read ', self.data_source, ' wrong')
        # 引入embedding矩阵和字典
        print("引入嵌入词典和矩阵")
